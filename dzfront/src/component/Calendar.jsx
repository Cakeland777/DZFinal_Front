  ...
  return (
    <div className="header row">
      <div className="col col-start">
        <span className="text">
          <span className="text month">{format(Month, "yyyy")}년</span>
          {format(Month, "MM")}월
        </span>
      </div>
    </div>
...
const ResetButton = ({ onReset }) => {
  return <button onClick={onReset}>일괄 해제</button>;
};
...
const Rendercell = ({
  ...
  let mapDate = {};
  //console.log(WorkDay);
  WorkDay.forEach((item) => (mapDate[item] = item));
      ...
      days.push(
        <div
          className={`col cell ${
            ...
            holidays.includes(calendar_day) ? "holiday" : ""
          }`} //달력에 현재 날짜 표시 여부
          onClick={dateCheck}
  ...
  const SelectAllButton = ({ onSelectAllClick }) => {
    return <button onClick={onSelectAllClick}>일괄 선택</button>;
  };
  // 해당 달의 모든 날짜 선택
...
export const Calendar = ({ payment_ym, workDate, worker_id, earner_code }) => {
  ...
  const [currentMonth, setCurrentMonth] = useState(
    parse(payment_ym, "yyyyMM", new Date())
  );
  const [commingMonth, setCommingMonth] = useState(
    addMonths(parse(payment_ym, "yyyyMM", new Date()), -1)
  );
